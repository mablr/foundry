...
use foundry_primitives::{AsSignableTx, FoundryTransactionBuilder};
...
impl<N, P> CastTxBuilder<N, P, InitState>
where
    N: Network,
    N::TransactionRequest: FoundryTransactionBuilder<N>,
    P: Provider<N>,
...
impl<N, P> CastTxBuilder<N, P, InputState>
where
    N: Network,
    ...
    P: Provider<N>,
{
    ...
    pub async fn build_unsigned_raw(self, from: Address) -> Result<String> {
        ...
        let mut typed_tx = tx.build_unsigned()?;
        let signable_tx = typed_tx
            .as_signable_tx()
            .ok_or_else(|| eyre::eyre!("transaction type does not support unsigned encoding"))?;
        Ok(hex::encode_prefixed(signable_tx.encoded_for_signing()))
    }
