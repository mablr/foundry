...
use foundry_primitives::{AsSignableTx, FoundryTransactionBuilder, sign_with_wallet};
...
impl MakeTxArgs {
        ...
        N::TransactionRequest: FoundryTransactionBuilder<N>,
    {
        ...
        let tx_envelope: N::TxEnvelope = if let Some(signable_tx) = unsigned_tx.as_signable_tx() {
            // Sign the transaction using the wallet's signer for the given sender address.
            let sig = sign_with_wallet(&EthereumWallet::new(signer), from, signable_tx).await?;
            let signed_tx = Signed::new_unhashed(unsigned_tx, sig);
            signed_tx.into()
        } else {
            unsigned_tx.into()
        };

        let signed_tx = hex::encode(tx_envelope.encoded_2718());
