...
impl ConstraintContext {
    ...
    fn normalize_bool(&self, cx: &mut SymCx, expr: SymBoolExpr) -> SymBoolExpr {
        let expr = if self.is_exact_value_constraint(&expr) {
            expr
        } else {
            expr.fold_exprs(cx, &mut |cx, expr| self.normalize_word(cx, expr))
        };
        let expr = normalize_bool_for_solver(cx, expr);
        match expr.kind() {
    ...
    fn exact_ceil_div_factor<'a>(&self, expr: &'a SymExpr) -> Option<(&'a SymExpr, U256)> {
        ...
        if rounding.as_const() != Some(denominator) || self.interval(numerator).is_none() {
            return None;
        }
    ...
    fn exact_scaled_div_factor<'a>(&self, expr: &'a SymExpr) -> Option<(&'a SymExpr, U256)> {
        ...
        if self.interval(numerator).is_none() || !(multiplier % denominator).is_zero() {
            return None;
        }
...
mod tests {
    ...
    fn direct_contradiction_uses_members_of_derived_positive_conjunction() {
        ...
        assert!(constraints_are_directly_unsat(&mut cx, &constraints), "{constraints:?}");
    }
    ...
    fn contextual_normalization_substitutes_direct_exact_values() {
        ...
        assert!(
            normalized.iter().any(|constraint| {
                matches!(
                constraint.kind(),
                SymBoolExprKind::Cmp(SymCmpOp::Ule, left, _)
                    if left.as_const() == Some(scale)
                )
            }),
            "{normalized:?}"
        );
