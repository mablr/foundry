...
use foundry_evm_core::{FoundryBlock, constants::DEFAULT_CREATE2_DEPLOYER};
...
impl Cheatcode for copyStorageCall {
    fn apply_stateful<CTX: ContextTr<Block: FoundryBlock, Journal: JournalExt>>(
        &self,
        ccx: &mut CheatsCtxt<'_, CTX>,
    ) -> Result {
...
/// from type strategy.
fn random_uint<BLOCK: FoundryBlock>(state: &mut Cheatcodes<BLOCK>, bits: Option<U256>, bounds: Option<(U256, U256)>) -> Result {
...
fn random_int<BLOCK: FoundryBlock>(state: &mut Cheatcodes<BLOCK>, bits: Option<U256>) -> Result {
...
fn get_canonical_type_def<BLOCK>(
    name_or_def: &String,
