...
use alloy_rpc_types::{
    BlockId, BlockNumberOrTag, TransactionInputKind, TransactionRequest, TransactionTrait, trace::parity::{Action, CreateAction, CreateOutput, TraceOutput}
};
    ...
    pub async fn run(mut self) -> Result<()> {
        ...
        if !self.ignore.is_some_and(|b| b.is_runtime()) {
            ...
            if let TxKind::Call(to) = transaction.kind() {
                ...
                tx_request.set_input_kind(Bytes::from(local_bytecode_vec), TransactionInputKind::Both);
            }
