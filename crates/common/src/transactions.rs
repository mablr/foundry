...
use alloy_network::{AnyNetwork, Ethereum, Network, TransactionBuilder, TransactionResponse};
...
impl From<alloy_network::AnyTransactionReceipt> for TransactionReceiptWithRevertReason<AnyNetwork> {
    fn from(receipt: alloy_network::AnyTransactionReceipt) -> Self {
        Self { receipt, revert_reason: None }
    }
}

impl From<TransactionReceiptWithRevertReason<AnyNetwork>> for alloy_network::AnyTransactionReceipt {
...
where
    N::TxEnvelope: UIfmt,
    N::TransactionRequest: FoundryTransactionBuilder<N> + UIfmt,
{
    fn pretty(&self) -> String {
        match self {
...
/// A transaction can either be an [`N::TransactionRequest`] waiting to be signed
/// or an [`N::TxEnvelope`], already signed.
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(untagged)]
#[serde(bound(
...
impl<N: Network> TransactionMaybeSigned<N> {
    ...
    {
        match self {
            Self::Signed(tx) => tx.recover_signer().ok(),
...
impl From<TransactionRequest> for TransactionMaybeSigned<Ethereum> {
    fn from(tx: TransactionRequest) -> Self {
        Self::Unsigned(tx)
    }
}

impl From<TxEnvelope> for TransactionMaybeSigned<Ethereum> {
