...
use alloy_network::{AnyNetwork, AnyRpcBlock, AnyTxEnvelope, BlockResponse, Network, TransactionResponse};
...
type ForkDB<N: Network> = CacheDB<SharedBackend<N>>;
...
impl<N: Network> Backend<N> {
    ...
    pub fn spawn(fork: Option<CreateFork>) -> eyre::Result<Self> {
        Self::new(MultiFork::<N>::spawn(), fork)
    }
    ...
    pub fn new(forks: MultiFork<N>, fork: Option<CreateFork>) -> eyre::Result<Self> {
    ...
    fn get_block_number_and_block_for_transaction(
        ...
        id: LocalForkId,
        transaction: B256,
    ) -> eyre::Result<(u64, <N as Network>::BlockResponse)> {
...
fn commit_transaction<N: Network>(
    tx: &<N as Network>::TransactionResponse,
    env: &mut EnvMut<'_>,
    journaled_state: &mut JournaledState,
    fork: &mut Fork<N>,
    fork_id: &ForkId,
...
mod tests {
    ...
    async fn can_read_write_cache() {
        ...
        let backend = Backend::<AnyNetwork>::spawn(Some(fork)).unwrap();
