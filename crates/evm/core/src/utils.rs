...
use alloy_network::{
    TransactionBuilder, TransactionBuilder4844, TransactionBuilder7702, TransactionResponse,
};
...
/// It actually doesn't make sense to have generic Network as the goal is to set Ethereum's EVM
/// TxEnv, so we need TransactionBuilder4844 + TransactionBuilder7702 traits bounds on transaction
/// request type.
pub fn configure_tx_req_env<
    N: Network,
    T: TransactionBuilder<N> + TransactionBuilder4844 + TransactionBuilder7702,
>(
    env: &mut EnvMut<'_>,
    tx: &T,
) -> eyre::Result<()> {
    env.tx.tx_type = tx.output_tx_type().into();
    ...
    // Type 3, EIP-4844
    // TODO: set blob versioned hashes
    // env.tx.blob_hashes = blob_versioned_hashes.clone().unwrap_or_default();
    env.tx.max_fee_per_blob_gas = tx.max_fee_per_blob_gas().unwrap_or_default();

    // Type 4, EIP-7702
    env.tx.set_signed_authorization(tx.authorization_list().cloned().unwrap_or_default());
