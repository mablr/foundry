...
use crate::{
    EthEvmCtx, FoundryBlock, FoundryInspectorExt, FoundryTransaction, TryAnyIntoTxEnv,
    ...
    Database<Error = DatabaseError> + DatabaseCommit + Debug
{
    ...
    fn transact(
        ...
        tx_env: TX,
        journaled_state: &mut JournaledState,
        inspector: &mut dyn for<'db> FoundryInspectorExt<EthEvmCtx<'db>>,
    ) -> eyre::Result<()>;
    ...
    N::TxEnvelope: TryAnyIntoTxEnv,
{
    ...
    pub fn inspect<I: for<'db> FoundryInspectorExt<EthEvmCtx<'db>>>(
        &mut self,
        evm_env: &mut EvmEnv,
        tx_env: &mut TxEnv,
        inspector: I,
