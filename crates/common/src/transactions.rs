...
use alloy_consensus::{Transaction, TxReceipt, TxEnvelope, transaction::SignerRecoverable};
    ...
    async fn fetch_revert_reason<P: Provider<FoundryNetwork>>(
        ...
        if let Some(block_hash) = self.receipt.block_hash() {
            let call_request = TransactionRequest {
                to: transaction.inner.to().map(TxKind::Call),
                from: Some(transaction.inner.from()),
                input: transaction.inner.input().clone().into(),
                ..Default::default()
            };
            let foundry_request = FoundryTransactionRequest::Op(WithOtherFields::new(call_request));
            match provider.call(foundry_request).block(BlockId::Hash(block_hash.into())).await {
...
pub fn get_pretty_tx_receipt_attr(
    ...
    match attr {
        "blockHash" | "block_hash" => Some(receipt.receipt.0.block_hash.pretty()),
        "blockNumber" | "block_number" => Some(receipt.receipt.0.block_number.pretty()),
        "contractAddress" | "contract_address" => Some(receipt.receipt.0.contract_address.pretty()),
        "cumulativeGasUsed" | "cumulative_gas_used" => {
            Some(receipt.receipt.0.inner.inner.cumulative_gas_used().pretty())
        }
        "effectiveGasPrice" | "effective_gas_price" => {
            Some(receipt.receipt.0.effective_gas_price().to_string())
        }
        "gasUsed" | "gas_used" => Some(receipt.receipt.0.gas_used().to_string()),
        "logs" => Some(receipt.receipt.0.inner.inner.logs().pretty()),
        "logsBloom" | "logs_bloom" => Some(receipt.receipt.0.inner.inner.bloom().pretty()),
        "root" | "stateRoot" | "state_root " => Some(receipt.receipt.0.state_root().pretty()),
        "status" | "statusCode" | "status_code" => {
            Some(receipt.receipt.0.inner.inner.status().pretty())
        }
        "transactionHash" | "transaction_hash" => Some(receipt.receipt.0.transaction_hash.pretty()),
        "transactionIndex" | "transaction_index" => {
            Some(receipt.receipt.0.transaction_index().pretty())
        }
