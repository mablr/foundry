    ...
    Database<Error = DatabaseError> + DatabaseCommit + Debug
{
    ...
    fn transact(
        ...
        evm_env: EvmEnv<SPEC, BLOCK>,
        journaled_state: &mut JournaledState,
        inspector: &mut dyn for<'db> FoundryInspectorExt<revm::Context<BLOCK, TX, CfgEnv<SPEC>, &'db mut dyn DatabaseExt<BLOCK, TX, SPEC>>>,
    ) -> eyre::Result<()>;
    ...
    N::TransactionResponse: TryAnyToTxEnv<F::Tx>,
{
    ...
    pub fn spawn(fork: Option<CreateFork>) -> eyre::Result<Self> {
        Self::new(MultiFork::<N, F::Spec, F::BlockEnv>::spawn(), fork)
    }
    ...
    pub fn set_spec_id(&mut self, spec_id: impl Into<F::Spec>) -> &mut Self {
    ...
    pub(crate) fn update_fork_db(
        ...
        target_fork: &mut Fork<N, F::BlockEnv>,
    ) {
    ...
    pub fn active_fork(&self) -> Option<&Fork<N, F::BlockEnv>> {
        self.active_fork_ids.map(|(_, idx)| self.inner.get_fork(idx))
    }

    /// Returns the currently active `Fork`, if any
    pub fn active_fork_mut(&mut self) -> Option<&mut Fork<N, F::BlockEnv>> {
        self.active_fork_ids.map(|(_, idx)| self.inner.get_fork_mut(idx))
    }

    /// Returns the currently active `ForkDB`, if any
    pub fn active_fork_db(&self) -> Option<&ForkDB<N, F::BlockEnv>> {
        self.active_fork().map(|f| &f.db)
    }

    /// Returns the currently active `ForkDB`, if any
    pub fn active_fork_db_mut(&mut self) -> Option<&mut ForkDB<N, F::BlockEnv>> {
    ...
    pub(crate) fn create_db_snapshot(&self) -> BackendDatabaseSnapshot<N, F::BlockEnv> {
    ...
    pub fn inspect<
        I: for<'db> FoundryInspectorExt<
            F::Context<&'db mut dyn DatabaseExt<F::BlockEnv, F::Tx, F::Spec>>,
        >,
    >(
        &mut self,
    ...
    where
        for<'db> F::Context<&'db mut dyn DatabaseExt<F::BlockEnv, F::Tx, F::Spec>>:
            FoundryContextExt,
    {
    ...
    N::TransactionResponse: TryAnyToTxEnv<F::Tx>,
{
    ...
    fn transact(
        ...
        journaled_state: &mut JournaledState,
        inspector: &mut dyn for<'db> FoundryInspectorExt<F::Context<&'db mut dyn DatabaseExt<F::BlockEnv, F::Tx, F::Spec>>>,
    ) -> eyre::Result<()> {
    ...
    fn transact_from_tx(
        ...
        let res = {
            ...
            let mut evm = F::default().create_evm_with_inspector(&mut db, evm_env, inspector);
...
#[derive(Clone, Debug)]
pub struct BackendInner<N: Network, F: FoundryEvmFactory> {
...
fn commit_transaction<'db, N: Network, F: FoundryEvmFactory>(
    ...
    fork_id: &ForkId,
    persistent_accounts: &HashSet<Address>,
    inspector: &mut dyn FoundryInspectorExt<F::Context<&'db mut dyn DatabaseExt<F::BlockEnv, F::Tx, F::Spec>>>,
) -> eyre::Result<()> 
where 
N::TransactionResponse: TryAnyToTxEnv<F::Tx>,
{
    ...
    let res = {
        ...
        let mut evm =
            F::default().create_foundry_evm(&mut db as _, evm_env.to_owned(), inspector);
