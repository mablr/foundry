...
impl<N> UIfmt for TransactionMaybeSigned<N>
where
    N: Network,
    N::TxEnvelope: UIfmt,
    N::TransactionRequest: foundry_primitives::FoundryTransactionBuilder<N>,
{
    fn pretty(&self) -> String {
        match self {
...
type                 {}
value                {}",
                tx.access_list().map(|a| a.pretty()).unwrap_or_default(),
                tx.chain_id().pretty(),
...
impl From<alloy_rpc_types::TransactionRequest> for TransactionMaybeSigned<Ethereum> {
    fn from(tx: alloy_rpc_types::TransactionRequest) -> Self {
        Self::new(tx)
    }
}

impl TryFrom<alloy_consensus::TxEnvelope> for TransactionMaybeSigned<Ethereum> {
    type Error = alloy_consensus::crypto::RecoveryError;

    fn try_from(tx: alloy_consensus::TxEnvelope) -> core::result::Result<Self, Self::Error> {
        Self::new_signed(tx)
    }
}
