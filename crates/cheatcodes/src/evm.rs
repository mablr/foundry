...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for addrCall {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getNonce_0Call {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getNonce_1Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for loadCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for loadAllocsCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for cloneAccountCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for dumpStateCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for recordCall {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for stopRecordCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for accessesCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for recordLogsCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getRecordedLogsCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getRecordedLogsJsonCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for pauseGasMeteringCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for resumeGasMeteringCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for resetGasMeteringCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for lastCallGasCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getChainIdCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for chainIdCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for coinbaseCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for difficultyCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for feeCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for prevrandao_0Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for prevrandao_1Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { newPrevrandao } = self;
        ensure!(
            *ccx.ecx.cfg().spec() >= SpecId::MERGE,
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for blobhashesCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getBlobhashesCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self {} = self;
        ensure!(
            *ccx.ecx.cfg().spec() >= SpecId::CANCUN,
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for rollCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getBlockNumberCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for txGasPriceCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for warpCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getBlockTimestampCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for blobBaseFeeCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getBlobBaseFeeCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for dealCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for etchCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for resetNonceCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { account } = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for setNonceCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for setNonceUnsafeCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { account, newNonce } = *self;
        let account = journaled_account(ccx.ecx, account)?;
        account.info.nonce = newNonce;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for storeCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for coolCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { target } = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for accessListCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for noAccessListCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for warmSlotCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { target, slot } = *self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for coolSlotCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { target, slot } = *self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for readCallersCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for snapshotValue_0Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for snapshotValue_1Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for snapshotGasLastCall_0Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { name } = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for snapshotGasLastCall_1Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for startSnapshotGas_0Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { name } = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for startSnapshotGas_1Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for stopSnapshotGas_0Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for stopSnapshotGas_1Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { name } = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for stopSnapshotGas_2Call {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { group, name } = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for snapshotCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for snapshotStateCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self {} = self;
        inner_snapshot_state(ccx)
    }
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for revertToCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for revertToStateCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { snapshotId } = self;
        inner_revert_to_state(ccx, *snapshotId)
    }
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for revertToAndDeleteCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { snapshotId } = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for revertToStateAndDeleteCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { snapshotId } = self;
        inner_revert_to_state_and_delete(ccx, *snapshotId)
    }
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for deleteSnapshotCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { snapshotId } = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for deleteStateSnapshotCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { snapshotId } = self;
        inner_delete_state_snapshot(ccx, *snapshotId)
    }
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for deleteSnapshotsCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for deleteStateSnapshotsCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self {} = self;
        inner_delete_state_snapshots(ccx)
    }
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for startStateDiffRecordingCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for stopAndReturnStateDiffCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
        let Self {} = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getStateDiffCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getStateDiffJsonCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let state_diffs = get_recorded_state_diffs(ccx);
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getStorageSlotsCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getStorageAccessesCall {
    fn apply(&self, state: &mut Cheatcodes<CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for broadcastRawTransactionCall
where
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for setBlockhashCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for executeTransactionCall {
    fn apply_full(
        &self,
        ccx: &mut CheatsCtxt<'_, CTX, N>,
        executor: &mut dyn CheatcodesExecutor<CTX, N>,
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for startDebugTraceRecordingCall {
    fn apply_full(
        &self,
        ccx: &mut CheatsCtxt<'_, CTX, N>,
        executor: &mut dyn CheatcodesExecutor<CTX, N>,
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for stopAndReturnDebugTraceRecordingCall {
    fn apply_full(
        &self,
        ccx: &mut CheatsCtxt<'_, CTX, N>,
        executor: &mut dyn CheatcodesExecutor<CTX, N>,
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for setEvmVersionCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let Self { evm } = self;
...
impl<CTX: EthCheatCtx, N: Network> Cheatcode<CTX, N> for getEvmVersionCall {
    fn apply_stateful(&self, ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
        let spec: SpecId = *ccx.ecx.cfg().spec();
...
pub(super) fn get_nonce<CTX: EthCheatCtx, N: Network>(
    ccx: &mut CheatsCtxt<'_, CTX, N>,
    address: &Address,
) -> Result {
...
fn inner_snapshot_state<CTX: EthCheatCtx, N: Network>(ccx: &mut CheatsCtxt<'_, CTX, N>) -> Result {
...
fn inner_revert_to_state<CTX: EthCheatCtx, N: Network>(
    ccx: &mut CheatsCtxt<'_, CTX, N>,
    snapshot_id: U256,
) -> Result {
...
fn inner_revert_to_state_and_delete<CTX: EthCheatCtx, N: Network>(
    ccx: &mut CheatsCtxt<'_, CTX, N>,
    snapshot_id: U256,
) -> Result {
    let mut evm_env = ccx.ecx.evm_clone();
...
fn inner_delete_state_snapshot<CTX: EthCheatCtx, N: Network>(
    ccx: &mut CheatsCtxt<'_, CTX, N>,
    snapshot_id: U256,
) -> Result {
...
fn inner_delete_state_snapshots<CTX: EthCheatCtx, N: Network>(
    ccx: &mut CheatsCtxt<'_, CTX, N>,
) -> Result {
...
fn inner_value_snapshot<CTX: EthCheatCtx, N: Network>(
    ccx: &mut CheatsCtxt<'_, CTX, N>,
    group: Option<String>,
    name: Option<String>,
    value: String,
...
fn inner_last_gas_snapshot<CTX: EthCheatCtx, N: Network>(
    ccx: &mut CheatsCtxt<'_, CTX, N>,
    group: Option<String>,
    name: Option<String>,
    value: u64,
...
fn inner_start_gas_snapshot<CTX: EthCheatCtx, N: Network>(
    ccx: &mut CheatsCtxt<'_, CTX, N>,
    group: Option<String>,
    name: Option<String>,
) -> Result {
...
fn inner_stop_gas_snapshot<CTX: EthCheatCtx, N: Network>(
    ccx: &mut CheatsCtxt<'_, CTX, N>,
    group: Option<String>,
    name: Option<String>,
) -> Result {
...
fn derive_snapshot_name<CTX: EthCheatCtx, N: Network>(
...
fn read_callers<CTX: EthCheatCtx, N: Network>(
...
fn get_state_diff<CTX: EthCheatCtx, N: Network>(state: &mut Cheatcodes<CTX, N>) -> Result {
    let res = state
...
fn get_recorded_state_diffs<CTX: EthCheatCtx, N: Network>(
    ccx: &mut CheatsCtxt<'_, CTX, N>,
...
fn get_contract_data<'a, CTX: EthCheatCtx, N: Network>(
...
fn set_cold_slot<CTX: EthCheatCtx, N: Network>(
    ccx: &mut CheatsCtxt<'_, CTX, N>,
    target: Address,
    slot: U256,
    cold: bool,
